...
to initialize a new npm project with sensible defaults. Now you can run
...
export default defineConfig({
  plugins: [laravel(["assets/css/app.css", "assets/js/app.js"])],
});
```
...
// Preload tags can be customized the same way:
setting('vite.preloadTagAttributes', [
    // Use `false` (or `null`) to omit an attribute entirely...
    'crossorigin' => false,
    'integrity'   => false,
]);
```
